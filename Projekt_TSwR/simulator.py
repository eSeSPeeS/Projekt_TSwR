...
simulator.py
============
Klasa F1tenthSimulator — łączy model dynamiczny, tor i wizualizację PyBullet.

Użycie:
    sim = F1tenthSimulator(track, params, use_pybullet=True)
    sim.reset()
    result = sim.run(controller, n_steps=500)
    sim.plot_trajectory()
    sim.animate()
"""
...
class F1tenthSimulator:
    ...
    Args:
        track:          tor (TrackCenterline)
        vehicle_params: parametry pojazdu (domyślne jeśli None)
        dt:             krok czasowy [s]
        use_pybullet:   czy otwierać okno PyBullet
    """
    ...
    def run(self, controller: Callable, n_steps: int,
        ...
        Args:
            controller: u = controller(state) → np.ndarray(2,)
            n_steps:    liczba kroków
            verbose:    logowanie co 100 kroków

        Returns:
            dict z kluczami: 'states', 'inputs', 'times'
        """
        oob = 0
    ...
    def _init_pybullet(self):
        try:
            ...
            pb.resetDebugVisualizerCamera(
                cameraDistance=8, cameraYaw=0, cameraPitch=-60,
    ...
    def plot_trajectory(self):
        """Trajektoria pojazdu na torze (kolorowana prędkością)."""
        ...
        lx, ly, rx, ry = track.get_track_boundaries()
        ...
        ax.plot(track.x, track.y, 'y--', lw=1, alpha=0.7, label='centerline')
    ...
    def plot_results(self):
        """Wykresy stanów i sterowań w czasie."""
        if len(self.state_history) < 2:
            print("Brak danych.")
            return
        ...
        state_labels  = ['s [m]', 'n [m]', 'mu [rad]',
                         'vx [m/s]', 'vy [m/s]', 'r [rad/s]']
        state_colors  = ['#2196F3', '#F44336', '#4CAF50',
    ...
    def animate(self, interval_ms: int = 30) -> FuncAnimation:
        ...
        ax.set_aspect('equal')
        ax.plot(track.x, track.y, 'y--', lw=1, alpha=0.6)
